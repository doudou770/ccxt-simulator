...
const (
	...
	bitgetRestURL        = "https://api.bitget.com"
	pingInterval         = 25 * time.Second
	reconnectDelay       = 5 * time.Second
	maxReconnectAttempts = 10
)

// Client is a Bitget WebSocket client
type Client struct {
	wsURL       string
	restURL     string
	conn        *websocket.Conn
...
// NewClient creates a new Bitget WebSocket client
func NewClient() *Client {
	return &Client{
		wsURL:      bitgetWSURL,
		restURL:    bitgetRestURL,
...
func (c *Client) Connect(ctx context.Context) error {
	...
	if err := c.loadSymbolInfo(); err != nil {
		log.Printf("[Bitget] Warning: failed to load symbol info: %v", err)
	}
...
func (c *Client) connect() error {
	...
	if err != nil {
		return fmt.Errorf("failed to connect to Bitget WebSocket: %w", err)
	}
	...
	log.Printf("[Bitget] WebSocket connected")
...
func (c *Client) subscribe(symbols []string) error {
	...
	log.Printf("[Bitget] Subscribed to %d symbols", len(symbols))
	return nil
}
	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure) {
				log.Printf("[Bitget] WebSocket error: %v", err)
			}
			c.handleDisconnect()
			continue
		}
...
func (c *Client) handleMessage(message []byte) {
	...
	if data.Arg.Channel != "ticker" || len(data.Data) == 0 {
		return
	}
	...
	for _, ticker := range data.Data {
		...
		symbol := c.convertToStandardSymbol(ticker.InstId)
...
func (c *Client) handleDisconnect() {
	...
	for c.reconnectAttempts < maxReconnectAttempts {
		...
		c.reconnectAttempts++
		log.Printf("[Bitget] Attempting reconnect %d/%d", c.reconnectAttempts, maxReconnectAttempts)

		if err := c.connect(); err != nil {
			log.Printf("[Bitget] Reconnect failed: %v", err)
			continue
		}
	...
	log.Printf("[Bitget] Max reconnect attempts reached")
}
	...
	for {
		...
		case <-ticker.C:
			...
			if err := conn.WriteMessage(websocket.TextMessage, []byte("ping")); err != nil {
				log.Printf("[Bitget] Ping failed: %v", err)
			}
		}
	}
}
...
func (c *Client) loadSymbolInfo() error {
	resp, err := http.Get(c.restURL + "/api/v2/mix/market/contracts?productType=USDT-FUTURES")
	if err != nil {
		return err
	}
	...
	var result struct {
		Data []struct {
			...
			PricePlace     int    `json:"pricePlace"`
	...
	for _, s := range result.Data {
		...
		maxQty, _ := strconv.ParseFloat(s.MaxTradeNum, 64)
		stepSize, _ := strconv.ParseFloat(s.SizeMultiplier, 64)

		info := &exchange.SymbolInfo{
			Symbol:            c.convertToStandardSymbol(s.Symbol),
			BaseAsset:         s.BaseCoin,
			QuoteAsset:        s.QuoteCoin,
			PricePrecision:    s.PricePlace,
			QuantityPrecision: s.VolumePlace,
			MinQty:            minQty,
			MaxQty:            maxQty,
			StepSize:          stepSize,
		}
	...
	log.Printf("[Bitget] Loaded %d symbols", len(c.symbols))
	return nil
}

// convertSymbol converts standard symbol (BTCUSDT) to Bitget format (BTCUSDT)
func (c *Client) convertSymbol(symbol string) string {
	return strings.ToUpper(symbol)
}

// convertToStandardSymbol converts Bitget format to standard symbol
...
func (c *Client) Close() error {
	...
	log.Printf("[Bitget] WebSocket closed")
	return nil
}

// GetCurrentPrice returns the current price
func (c *Client) GetCurrentPrice(symbol string) (float64, error) {
	url := fmt.Sprintf("%s/api/v2/mix/market/ticker?symbol=%s&productType=USDT-FUTURES", c.restURL, c.convertSymbol(symbol))
	...
	return strconv.ParseFloat(result.Data[0].MarkPrice, 64)
}
